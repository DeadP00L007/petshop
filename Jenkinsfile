pipeline{
    ...
    stages{
        ...
        stage ('checkout SCM') {
            steps {
                git 'https://github.com/DeadP00L007/petshop.git'
            }
        }
            ...
            steps{
                script{
                    withDockerRegistry(credentialsId: 'docker', toolName: 'docker') {
                        sh "docker build -t dcx369/petshop:${BUILD_TAG} ."
                        sh "docker push dcx369/petshop:${BUILD_TAG}"
                   }
                }
            }
        }
        stage("Image Scanning using TRIVY"){
            ...
            steps{
                ...
                sh 'docker run -d --name qacontainer -p 8090:8080 dcx369/petshop:latest'
                sleep time: 60, unit: 'SECONDS'
                retry(10){
                
                sh 'curl --silent http://localhost:8090/jpetstore/ | grep JPetStore'
                }
                //testing curl
     ...
     always {
        emailext attachLog: true,
            ...
            body: "Project: ${env.JOB_NAME}<br/>" +
                ...
                "URL: ${env.BUILD_URL}<br/>",
            to: 'dhamichiran@gmail.com',  // Developer or manager Email
            attachmentsPattern: 'trivy.txt'
